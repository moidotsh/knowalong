...
- French tense/mood inventory: present tense, passé composé (the everyday perfective past — "j'ai fait"), imparfait (imperfect — ongoing/habitual past — "je faisais"), passé simple (literary perfective past — "je fis"), plus-que-parfait (pluperfect — "j'avais fait"), futur simple (future — "je ferai"), passé antérieur (literary anterior past), conditionnel présent/passé (conditional), subjonctif présent/passé (subjunctive), impératif (imperative), infinitif présent/passé (infinitive), participe présent/passé (participle). Pick ONE tense/mood per form.
- Infinitives end in -er (1st group: parler, aimer), -ir (2nd group: finir, choisir; note many -ir verbs are 3rd-group irregulars: partir, venir), -re (3rd group: prendre, vendre), or -oir (3rd group: avoir, pouvoir, savoir). Note them as "infinitive"; never mark an infinitive for tense/person/number/case.
- French has NO grammatical case. Never write "nominative"/"accusative"/"genitive"/"dative" for a French form. (Object relations are expressed via word order and prepositions, especially à and de.)
- Negation: bipartite "ne...pas" (or "ne...plus/rien/jamais/personne/que"). The "ne" (or n' before a vowel) is a clitic before the verb; the second element follows the verb (or auxiliary). In colloquial spoken French the "ne" is often dropped — note register when relevant.
- Elision: the vowel-final short words (je, me, te, le, la, ne, que, se, ce, de) elide to j'/m'/t'/l'/n'/qu'/s'/c'/d' before a vowel or mute h. Note elision in the grammaticalNote when exemplified (e.g. "j'ai", "l'ami").
- Liaison: a silent final consonant is pronounced before a following vowel (les amis → /le.z‿a.mi/). Note liaison when it exemplifies a concept; the liaison consonant is NOT part of the surface form spelling.
- Articles: definite (le, la, les), indefinite (un, une, des), partitive (du, de la, des). Contractions: à + le = au, à + les = aux, de + le = du, de + les = des. Note the article type when relevant.
- Pronouns: subject (je, tu, il, elle, on, nous, vous, ils, elles), direct object (me, te, le, la, les), indirect object (me, te, lui, nous, vous, leur), y (location/proposition), en (quantity/origin), stressed (moi, toi, lui, elle, soi, nous, vous, eux, elles). Pick ONE slot per form.
- Past participle agreement: the past participle agrees with the subject for verbs conjugated with "être" (aller, venir, naître, mourir, reflexives, etc.), and with a preceding direct object for verbs conjugated with "avoir". Note the agreement explicitly when relevant.
- Register: note formal (soutenu), standard, or familiar when a form is register-marked (e.g. "je ne sais pas" soutenu vs "je sais pas" familiar — dropping "ne").
- Compound verbs and auxiliaries: note when a form uses "être" vs "avoir" as the auxiliary in passé composé.
- Subjunctive triggers: note when a form exemplifies a subjunctive trigger (volition, emotion, doubt, impersonal expressions like "il faut que").`,
    inventedWordAntiExamples: ['faisaison', 'parlerer'],
    stage3OrthographyAntiPattern:
      '- For French sourceText: do NOT use Cyrillic letters (U+0400-U+04FF — Russian text bleed). Do NOT use Greek letters (U+0370-U+03FF). Do NOT use Arabic letters (U+0600-U+06FF) or Hebrew (U+0590-U+05FF). Do NOT use non-French Latin confusables ñ (U+00F1 — Spanish) or ß (U+00DF — German) or ð/þ (U+00F0/U+00FE — Icelandic); French uses "gn" for /ɲ (not "ñ") and "ss" or "B" capital for /s/ (not "ß"). Do NOT double the infinitive suffix (parlerer WRONG; parler RIGHT; finirir WRONG; finir RIGHT). Do NOT mix English words into French sentences ("Je suis going" WRONG).',
    fewShotRealizations: `Examples of well-formed entries (do NOT copy these concepts — only use them as shape reference):
