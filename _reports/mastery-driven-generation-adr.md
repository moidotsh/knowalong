...
| **4** | Retire static song packs: the section screen generates lessons on the fly from `svetoforSong.ts` targets (narrative order) + the generator. Dense `V1/CH/V2/O` lessons removed; `buildArcLessons`/`INTRO_LESSONS` superseded by the dynamic path. | Svetofor sections render dynamic, cap-compliant lessons; Study + CLCC decks unaffected; full audit + `tsc` + `build:web` green. |
| **5** | Mastery-gated culminating lines + lyric-teaser UI (the existing mosaic becomes the read-only teaser; the culminating-line lesson locks until all constituent surface forms are graduated). | A fresh learner cannot open a culminating line; a learner with all line-forms graduated can. |
